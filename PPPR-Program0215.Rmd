...
# permit2021_join <- left_join(perimit2021, FacilityID, by =c("FacilityID")) %>% 
#  left_join(., pprProperties, by =c("PPR_Properties_ObjectID"="OBJECTID"))
...
# District 7 Facility Location Map & Service Radius
...
ggplot(Facility_7)+
  ...
  geom_sf(aes(geometry = geometry),color = palette1_main,fill = palette1_main,alpha = 0.8) +
  labs(title="Parks w/ Programs in District 7")
  mapTheme()
...
# District 8 Facility Location Map & Service Radius
...
ggplot(Facility_8)+
  ...
  geom_sf(data=st_union(pprDestServiceArea %>% filter(PPR_DIST ==8)),color="black",size=1,fill = "transparent")+
  geom_sf(data=st_union(pprDestServiceArea %>% filter(PPR_DIST ==9)),color="black",size=1,fill = "transparent")+
  geom_sf(aes(geometry = geometry),color = palette1_main,fill = palette1_main,alpha = 0.3) +
  labs(title="Parks w/ Programs in District 8")
  mapTheme()
...
# District 9 Facility Location Map & Service Radius
...
ggplot(Facility_9)+
  ...
  geom_sf(data=st_union(pprDestServiceArea %>% filter(PPR_DIST ==9)),color="black",size=1,fill = "transparent")+
  geom_sf(aes(geometry = geometry),color = palette1_main,fill = palette1_main,alpha = 0.3) +
  labs(title="Parks w/ Programs in District 9")

  mapTheme()


#Buffers_9 <- buffer(Facility_9, width=1, dissolve=TRUE)

#Buffers_9_1mile = st_buffer(unique(Facility_9), dist =1600)

#buffer <- filter(B_Buffers, Legend=="Unioned Buffer")

#gBuffer(Facility_9, byid=FALSE, id=NULL, width=1.0, quadsegs=5, capStyle="ROUND",
