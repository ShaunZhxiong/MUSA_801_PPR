...
moves2018 <- vroom("data/safegraph/OneDrive-2021-12-07/moves_2018.csv")
colnames(moves2018)
```

```{r moves2020, message=FALSE, warning=FALSE}
moves_monthly2020 <- vroom("data/safegraph/OneDrive-2021-12-07/moves_monthly2020.csv")
...
This above program data is not intact
...
# count poisition number
m <- safeGraph.join %>% 
  dplyr::select(placekey) %>% 
  n_distinct() 
print(paste0("Position number with spatial join method is  ",m))

n <- parks %>% 
  dplyr::select(placekey) %>% 
  n_distinct()
print(paste0("Position number with filtering categories is  ",n))

left_join(parks, propertyParent, by=c(""))
```

```{r}
safeGraph.join2 <-  propertyArea %>% 
  st_join(parks %>% dplyr::select(placekey, geometry) %>% distinct(), left=FALSE) %>% 
            dplyr::select(PUBLIC_NAME) %>% 
            distinct()

safeGraph.join2 %>% n_distinct()

key <- pprProperties %>% 
  dplyr::select(ALIAS_ADDRESS, geometry) %>% 
  na.omit()
```
...
st_write(obj = s, dsn = conn, Id(table = "philly"))
# writing to postgres
# dbListTables(conn)
```
