...
modelData <- left_join(modelData, modelData.prob,
                       by=c("placekey", "origin"))
...
f <- function(col_name, df) {
  x <- df %>% filter({col_name} > 100)
  return(x)
} 

x <- f('ACREAGE', modelData)
```
