...
```{r join property data,message=FALSE,warning=FALSE}
# # replace "and" with "&"
...
```{r join safegraph data,warning=FALSE,message=FALSE,eval=FALSE}
# filter into philly
...
```{r filter safegraph data, eval=FALSE, message=FALSE, warning=FALSE}
...
```{r readBackup PPRmoves, include=FALSE,warning=FALSE,message=FALSE}
...
```{r readBackupprogram2021.joinWithPlaceKey,warning=FALSE,message=FALSE,include=FALSE}
...
```{r unnest safegraph data, message=FALSE, warning=FALSE,include=FALSE}
...
```{r census-exploration-1, warning=FALSE, message=FALSE}
...
```{r getCensusTracts, warning=FALSE, message=FALSE,include=FALSE}
...
```{r load data into database, eval=FALSE, include=FALSE}
library(RPostgres)
...
df_to_db(conn, propertyWithPlaceKey, "ppr_properties")
# writing to postgres
# dbListTables(conn)
```

```{r load pprTrails data into database, eval=FALSE, include=FALSE}
# print(pprTrails)
```
...
#### The facility with largest visit counts

From above, we can know Christy Recreation Center([zzz-222\@63s-dvq-5fz](mailto:zzz-222@63s-dvq-5fz){.email}) has the most frequent visits among district 7,8,9. The total visits is 420776
...
From the above plot we can see the largest dwelling time is at Girard Park ([zzz-222\@628-pgb-k75](mailto:zzz-222@628-pgb-k75){.email}) with avgerage dwelling time is 94.9. Interestingly, there is not program by PPR in this park in 2021. There is a activity organized by citizens themselves. The activity is yoga, from April to September, every 1pm to 2pm on Sunday.

#### The facility with largest average dwell time

```{r Girard Park img, echo=FALSE,fig.height = 1,out.width = '30%',fig.align = 'center', fig.cap = 'Figure Matthias Baldwin Park'}
knitr::include_graphics("resources/imgaes/Girard Park.png")
```

```{r Girard Park dwell time ,fig.height = 5,message = FALSE, warning = FALSE,out.width ='75%',fig.align = 'center',fig.cap = 'Figure. Map of dwell time'}
sumVisit <- dwellTime %>% 
...
```{r temperal pattern by month, eval=FALSE, include=FALSE}
sumVisit <- visitCount %>% 
...
```{r Girard Park temperal pattern by day ,fig.height = 5,message = FALSE, warning = FALSE,out.width ='75%',fig.align = 'center',fig.cap = 'Figure. Map of dwell time'}
visitCount %>% 
  filter(placekey=='zzz-222@628-pgb-k75') %>% 
  st_drop_geometry()%>%
  na.omit()%>%
...
```{r Girard Park temperal pattern by hour,fig.height = 5,message = FALSE, warning = FALSE,out.width ='75%',fig.align = 'center',fig.cap = 'Figure. Map of visit time'}
sumVisit <- visitHour %>% 
  filter(placekey=='zzz-222@628-pgb-k75') %>% 
  dplyr::select(-month) %>% 
  group_by(hour) %>%
...
```{r temperal pattern by weekday, eval=FALSE, include=FALSE}
...
#### Cluster analysis for locations

##### Data confidence clusters

```{r integrate safegraph characteristic data}
...
# visitHour, originCount, departCount, dwellTime, visitWeekday
...
# normalize
visitIntegrated.scale <- scale(visitIntegrated %>% dplyr::select(-placekey, -geometry))

# cluster number
wss <- (nrow(visitIntegrated.scale)-1)*sum(apply(visitIntegrated.scale,2,var)) 

for (i in 2:15) wss[i] <- sum(kmeans(visitIntegrated.scale, 
   centers=i)$withinss) 
...
# K-Means cluster
fit1 <- kmeans(visitIntegrated.scale, 3) 
summary(fit1)
# integrate 
visitIntegrated <- visitIntegrated %>% 
  mutate(cluster = fit1$cluster)
# mean by cluster
fit1_mean <- aggregate(visitIntegrated%>% dplyr::select(-placekey, -geometry),
                       by=list(visitIntegrated$cluster),FUN=mean)

fit1_mean 
```

As the result, most locations belong to cluster 2, the normal one. Cluster 1 is with extremely high night visit and visit longer than 4h. Cluster 3 is with many transitory visit.

```{r clusterplot of cluster analysis 1}
clusplot(visitIntegrated.scale, 
         fit1$cluster, color=TRUE, shade=TRUE,
   labels=2, lines=0)
```

```{r dwell time for cluster analysis 1}
visitIntegrated %>%
  mutate(cluster=as.character(cluster)) %>% 
  dplyr::select(-placekey, -nightVisit, -geometry) %>% 
  gather(dwellTimes, visitors, -cluster) %>%
  ggplot(aes(dwellTimes,visitors)) +
  geom_bar(position ="dodge",stat='identity', aes(fill=cluster)) +
  labs(x="Dwell Time", y="Aggregated Visitors",
       title ='') +
  scale_fill_manual(values = palette5[1:3])+
  plotTheme(10,20)
```

```{r night visit for cluster analysis 1}
fit1_mean %>%
  mutate(cluster=as.character(Group.1)) %>% 
  dplyr::select(nightVisit, cluster) %>% 
  ggplot(aes(cluster,nightVisit)) +
  geom_bar(position ="dodge",stat='identity', aes(fill=cluster)) +
  labs(x="Cluster", y="Average Night Visits",
       title ='') +
  scale_fill_manual(values = palette5[1:3])+
  plotTheme(10,20)
```
...
```{r integrate safegraph data with program}
#inner join
SGwithProgram <- st_join(visitCount %>% 
                           group_by(placekey) %>% 
                         ...
                         program2021.join %>% 
                           st_sf() %>% 
                           group_by(FacilityID) %>% 
                           summarize(programCount = n()) %>% 
                           st_sf(),
                         left=FALSE)
```

```{r cluster by conflict}
# normalize
SGwithProgram.scale <- scale(SGwithProgram %>% 
                               dplyr::select(visits, programCount) %>% 
                               st_drop_geometry())

# cluster number
wss2 <- (nrow(SGwithProgram.scale)-1)*sum(apply(SGwithProgram.scale,2,var)) 

for (i in 2:15) wss2[i] <- sum(kmeans(SGwithProgram.scale, 
   centers=i)$withinss) 
...
# K-Means cluster
fit2 <- kmeans(SGwithProgram.scale, 3) 
summary(fit2)
# integrate 
SGwithProgram <- SGwithProgram %>% 
  mutate(cluster = fit2$cluster)
# mean by cluster
fit2_mean <- aggregate(SGwithProgram%>% dplyr::select(-placekey, -FacilityID, -cluster) %>% st_drop_geometry(),
                       by=list(SGwithProgram$cluster),FUN=mean) %>% 
  mutate(cluster = as.character(Group.1)) %>% 
...
cluster 2 has high visits but less programs. on the other hand cluster 1 has a lot programs and few vistis. Cluster2 ({AC45E5EF-2467-4926-A1DA-07258305D871). Christy Recreation Center AGAIN!!!

```{r visit for clouster analysis 2}
fit2_mean %>%
  dplyr::select(visits, cluster) %>% 
  ggplot(aes(cluster,visits)) +
  geom_bar(position ="dodge",stat='identity', aes(fill=cluster)) +
  labs(title ='') +
  scale_fill_manual(values = palette5[1:3])+
  plotTheme(10,20)
...
```{r preogeam counts for cluster analysis 2}
fit2_mean %>%
  dplyr::select(programCount, cluster) %>% 
  ggplot(aes(cluster,programCount)) +
  geom_bar(position ="dodge",stat='identity', aes(fill=cluster)) +
  labs(title ='') +
  scale_fill_manual(values = palette5[1:3])+
  plotTheme(10,20)
...
```{r scater plot for cluster analysis 2}
ggplot(SGwithProgram %>% 
         mutate(cluster = as.character(cluster)), 
       aes(programCount, visits, color=cluster)) +
  geom_point(size = 1)+
  geom_smooth(method = "lm", se = FALSE, colour = palette1_assist) +
  labs(title ="") +
  scale_color_manual(values = palette5[1:3])+
  plotTheme()

ggplot(data.frame(SGwithProgram.scale) %>% 
         mutate(cluster = fit2$cluster,
                cluster = as.character(cluster)), 
       aes(programCount, visits, color=cluster)) +
  geom_point(size = 1)+
  geom_smooth(method = "lm", se = FALSE, colour = palette5[4]) +
  labs(title ="") +
  scale_color_manual(values = palette5[1:3])+
  plotTheme()
```

zzz-222\@63s-dt9-3kf - Eastwick Regional Park

![](images/paste-5AA4C43C.png)

```{r dwell zzz-222@63s-dt9-3kf, fig.height = 5,message = FALSE, warning = FALSE,out.width ='75%'}

sumVisit <- dwellTime %>% 
  filter(placekey=='zzz-222@63s-dt9-3kf') %>% 
  dplyr::select(-month) %>% 
  group_by(dwellTimes) %>%
  summarise(visitors=sum(visitors)) %>% 
  st_drop_geometry() 
...
sumVisit%>%
  ...
  geom_bar(position ="dodge",fill = palette1_main,stat='identity') +
  labs(x="Dwell Time", y="Aggregated Visitors",
       title ='Girard Park (zzz-222@63s-dt9-3kf )') +
  plotTheme(10,20)
```

```{r visit zzz-222@63s-dt9-3kf, fig.height = 5,message = FALSE, warning = FALSE,out.width ='75%'}
visitCount %>% 
  filter(placekey=='zzz-222@63s-dt9-3kf') %>% 
  st_drop_geometry()%>%
  na.omit()%>%
  ggplot(aes(visitDay,visits)) + 
  geom_line(color=palette1_main,size=1)+
  labs(title = 'Eastwick Regional Park (zzz-222@63s-dt9-3kf)',x="Visit Date",y="Safegraph Visit")+
  plotTheme(10,20)+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))
```

```{r  hour zzz-222@63s-dt9-3kf }
sumVisit <- visitHour %>% 
  filter(placekey=='zzz-222@63s-dt9-3kf') %>% 
  dplyr::select(-month) %>% 
  group_by(hour) %>%
...
sumVisit%>%
  ...
  geom_bar(position ="dodge",fill = palette1_main,stat='identity') +
  labs(x="hour", y="Aggregated Visits",
       title ='Eastwick Regional Park (zzz-222@63s-dt9-3kf)') +
  plotTheme(10,20)
```

```{r zzz-222@63s-dt9-3kf program}
Eastwick <- program2021.join %>% 
  filter(FacilityID == "{9D68AC8D-CF45-49EC-81E3-022EF3203A04}")


EastwickSum <- Eastwick %>%
...
EastwickSum %>%
  ...
  labs(y = "Visitor Count", fill="ActivityType", title = "Eastwickr Activity with Total ")
```
