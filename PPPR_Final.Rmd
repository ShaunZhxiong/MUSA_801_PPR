...
In Figure 3.2.9, we calculate the average dwelling time for each facility. From the map, we can find out that the **longest dwelling** time happes at the placekey ([zzw-222\@628-pg9-tg](mailto:zzw-222@628-pg9-tg){.email}), the **Clemente Park Playground**.
...
```{r general fit parameter, message = FALSE, warning = FALSE, results='hide'}
...
```{r predict in SCENARIO 1, message=FALSE, warning=FALSE}
# SCENARIO 1: Drop NA probability and use large PCA
...
print(paste0("MAE for Scenario 1 is ",round(mean(error$MAE,na.rm=T),4)))
print(paste0("MAPE for Scenario 1 is ",round(mean(error$MAPE,na.rm=T),4)))
```

##### 7.2.1 Scenario 3

```{r predict in sCENARIO 3, message=FALSE, warning=FALSE}
...
print(paste0("MAE for Scenario 3 is ",round(mean(error$MAE,na.rm=T),4)))
```
...
```{r scenario1FrequencyResult, message=FALSE, warning=FALSE}
kable(scenario1FrequencyResult,align = 'c',caption = '<center>Table 7.3.1 Prediction Error of Scenario 1 in different visit frequency <center/>') %>%
  kable_classic(full_width = T)%>%
  kable_styling(position = "center")
```
...
```{r add visits frequency level into the model, message=FALSE, warning=FALSE}
...
outcomeAnalysis(parameter,"Final Optimal Model:
                Dropping NA probability, using PCA, and taking visits frequency level into consideration")
```
...
In the following, we will look at the accuracy for the prediction to see if the prediction accuracy has sucessfully improve.

##### 7.3.2 Prediction

```{r add visits frequency level into the model2, message=FALSE, warning=FALSE}
# avoid error:contrasts can be applied only to factors with 2 or more levels
Test <- Test %>%
...
print(paste0("MAE is ",round(mean(predictionResult_group$MAE,na.rm=T),4)))
print(paste0("MAPE is ",round(mean(predictionResult_group$MAPE,na.rm=T),4)))
```

The MAE has decreased from 0.06 to 0.044, that is a impressing improve. And the MAPE also decreases from 3.34 in 7.2.1 to 1.46. The testing accuracy means that the model optimization also helps improve the prediction accuracy. 
...
In the above, we can see that as we introduce the frequency categorical variable into the model, the accuracy improves a lot. 

#### 7.4 Cross Validation - Spatial Genaralizability

need modification

```{r CV before optimalization over district}
...
```{r CV after optimalization over district}
...
fit <- fit_parameter_level(data = xx,
                           places = RealModelPlaces_final %>% filter(OBJECTID%in%unique(xx$OBJECTID)),
                           neighbor_data = convenientSurroundEffect,
                           neighbor_id_column="placekey",
                           neighbor_attr_column ="visits",
                           id_column="OBJECTID",
...
For the use case, we can help PPR better understand the use of their facilities and effect of rearranging the program numbers. Each time when PPR rearrange the number of programs, the market area will