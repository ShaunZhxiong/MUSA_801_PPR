...
```{r openData_pprDistrictAndServiceArea, fig.align='center', fig.cap='Location of PPR Districts and Pilot Service Areas', fig.height=8, message=FALSE, warning=FALSE, out.width='80%'}
...
```{r programNpermit,fig.height = 6,message = FALSE, warning = FALSE, results='hide',out.width = '80%',fig.align = 'center',fig.cap = 'Figure2.1.1 Facilities w/ Programmed (red) & Permited (orange) Activities across Philly'}
...
```{r SA9_Events 2,fig.height = 8,message = FALSE, warning = FALSE, results='hide',out.width = '80%',fig.align = 'center',fig.cap = 'Figure2.2.10 Visitor Counts by Week and Activity Categories in District 9'}
...
```{r Thomas B. Smith Recreation Center img, echo=FALSE,fig.height = 2,out.width = '80%',fig.align = 'center'}
...
```{r Thomas B. Smith Recreation Center dwell time ,fig.height = 3,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure3.3.3 Smith Rec Center Activity with Total'}
...
####4.3 Special Cases2:Girard Estates Park
...
In the next phase, we will focus on **creating the Huff model in District 7,8,9**, and try to define and construct the attractiveness metric of facility. This attractiveness metric will be used in the huff model to predict the probability of visitors will stop by each facility. Finally we will compare the current program distribution with the visit probabilities and market areas of each position to suggest the PPR allocating programmings and serving target residents.

### 6 Huff Model with Neighbor Effect

#### 6.1 Normalized data

The number of device residing from the SafeGraph data is a sample of device users tracked by app carriers, which cannot be considered as all the visitors to certain park facilities from a census block group. According to [SafeGraph's guidelines for normalizing Patterns data](https://colab.research.google.com/drive/1lTi8JXfX9rh2mnuFjgYgKsIcMcDI3EG_?usp=sharing#scrollTo=aHTXQFdA7A2y), we normalized the number of devices residing by multiplying a parameter of total census block group (CBG) population divided by average device residing in this CBG. In that case, the outcome can estimate all the visitors from a CBG to certain park facilities. Since the PPR doesn't schedule programs in some facilities, we exclud them (168 out of 524 facilities) on the exception list in our further analysis. 
...
We collected many features of facilities as attractiveness, including: area, facility number, facility area, trail miles, picnic table, exercise number, swimming pool number, spray ground number, hydration number, tennis court number, playground number, tree height, tree area, program number and capacity, and permit number and capacity. 

```{r calculate attractiveness, message=FALSE, warning=FALSE, echo=FALSE, fig.height = 12,message = FALSE, warning = FALSE,out.width ='200%',fig.align = 'center',fig.cap = 'Figure6.2.1 Summary of Attractiveness' }
attractivenssData <- 
  modelData %>% 
...
With correlation analysis, we find facility area, facility number and trail miles are highly correlated. Then we will do Prominent Component Analysis(PCA) to reduce features' correlation.
```{r calculate correlation, fig.height = 8,message = FALSE, warning = FALSE, echo=FALSE, out.width ='80%',fig.align = 'center',fig.cap = 'Figure6.2.2 Correlation of Attractiveness'}
attracGEO <- 
...
```
We will do feature engineering with two methods.
...
OSRM (Open Source Routing Machine) is a C++ implementation of a high performance route search engine to obtain the shortest paths in a road network. We use it to calculate the shortest distance from origin places to destination facilities by mobile. 
...
The observed probability that visitors from a census block group (CBG) choose a facility as destination can be calculated with normalized number of visitors to a facility divided by the CBG population. One limitation is some population data is missing in census data, so the probability of visitors from these CBG to a certain park will be NA (not available). The highlighted areas in figure 6.4.1 are census block groups with no population data.

```{r Wrangle sg and property data for model, fig.height = 8,message = FALSE, warning = FALSE, results='hide',out.width = '80%',fig.align = 'center',fig.cap = 'Figure6.4.1 Census Block Group with Missing Data'}
...
Centrality is the term used to describe the neighboring effect of attraction. We ranked POIs with their NAICS codes and find the top two are convenience stores (445120) and restaurants (722513), then we will use them as two Centrality factors which affect visitors' choice the park facilities.

```{r centrality, fig.height = 3,message = FALSE, warning = FALSE,out.width ='100%',fig.align = 'center',fig.cap = 'Figure6.5.1 Total Vistors of Related Brand With Naics'}
...
```{r visualize centrality, fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure6.5.2 Overlay Map of Parks(black), Restaurants(red) and Convinient Stores(orange)'}
...
```{r model1 fit parameter, fig.height = 3,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure6.6.1 Histogram of data set1'}
##### Prepare data
...
```{r, 2model fit parameter, fig.height = 3,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure6.6.2 Histogram of data set1'}
...
```{r huff model3 fit parameter, fig.height = 3,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure6.6.3 Histogram of data set3'}
...
```{r model huff4 fit parameter, fig.height = 3,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure6.6.4 Histogram of data set4'}
...
We divide the data based on record dates into four seasons and run individual model can predict the probability in each season. The red line represent temperature while the orange line represent number of visitors. Both number are exaggerate 500 times for visualization.

```{r temporal chart, fig.height = 3,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure6.7.1 Temporal Chart of vistor and temperature'}
# temperature data
weather.Data <-
...
```{r missing data for model, fig.height = 8,message = FALSE, warning = FALSE, results='hide',out.width = '80%',fig.align = 'center',fig.cap = 'Figure6.7.2 Census Block Group with Missing Probability'}
# calculate probability 
...
```{r probability map, fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure6.7.3 Probability Map with Missing Data'}
