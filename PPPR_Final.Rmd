...
```{r hist of MAPE for parks,fig.height = 4,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure7.3.2.3 Distribution Histogram of MAE & MAPE for Different Parks & Rec Centers'}
ggplot() + 
  geom_sf(data=left_join(predictionResult_group, CensusPoly, by=c("origin"="GEOID")) %>% st_sf(),
          color="black",size=0.05,aes(fill = q5(MAPE)))+
...
m1 <- ggplot() + 
  geom_sf(data=cv_group_result_dist,
          color="black",size=0.05,aes(fill=MAPE))+
  geom_sf(data=pprDistrict %>% st_union(),color="black",size=0.5,fill = "transparent")+
