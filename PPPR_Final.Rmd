...
propertyWithPlaceKey <- st_join(property %>% filter(NESTED == "N") %>% st_transform(crs=32118),
                                st_buffer(parks %>% drop_na() %>% dplyr::select(placekey, geometry) %>% st_transform(crs=32118),10),left=FALSE) %>%
  st_drop_geometry() %>% 
...
program2021.joinWithPlaceKey <- 
  st_join(program2021.join %>%
            ...
            st_buffer(10),
          propertyWithPlaceKey%>% st_transform(crs=32118) %>% dplyr::select(placekey, geometry),left=FALSE) %>%
  st_drop_geometry() %>% 
...
# write.csv(homePanelToGo,"data/output/normalizationQuotient.csv")
...
sgIntegrated_byMonth <- full_join(dwellMean %>% dplyr::select(placekey,month, meanDwell), 
  ...
  left_join(program2021.joinWithPlaceKey %>% 
              ...
              group_by(placekey, month, PPR_DIST) %>% 
...
sgIntegrated_byMonth.scale <- scale(sgIntegrated_byMonth %>% 
                                      st_drop_geometry() %>% 
                                      dplyr::select(-placekey,-PPR_DIST, -month))
...
cluster2_mean <- aggregate(sgIntegrated_byMonth %>% 
                             dplyr::select(-placekey, -cluster,-month,-PPR_DIST) %>% 
                             st_drop_geometry(),
...
kable(cluster2_mean %>% 
        mutate(color=c("pink","orange","blue","orange"),.before = 2) %>% 
        mutate(group=c("potential", "normal","over-programmed", "normal long-stay"),.before = 2),
...
#Color points by groups
my_cols <- c(palette1_main,palette5[1],palette5[5],palette5[1])

pairs(sgIntegrated_byMonth %>% 
        st_drop_geometry() %>% 
        dplyr::select(meanDwell,visitors,programs), 
      pch = 19, cex = 0.5, cex.labels=1, diag.panel = panel.hist,
      size=0.5,
