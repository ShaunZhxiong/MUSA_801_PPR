...
ggplot(Facility_Program %>%filter(PPR_DISTRICT == 9)) +
  ...
  labs(y = "Program Frequency", fill="Category", title = "Categories of Events in District8")+
  plotTheme()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8),
        legend.position = "bottom",
        legend.text = element_text( colour = "black", size = 8))
...
```{r cluster by data confidence2, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r exceptionProperty, message=FALSE, warning=FALSE, results='hide'}
...
```{r spatially distribution of sg bias clusters,fig.height = 12,message = FALSE, warning = FALSE,out.width ='100%',fig.align = 'center',fig.cap = 'Figure 4.1.2 Map of Bias Type of SafeGraph Data'}
...
(3) People tend to choose travel destinations with more beneficial future choices. For example, a tourist at Origin O has two destination choices A and B, with equal distance to origin O. In this case, destination B should be preferred since it has more future choices in its neighborhood compared with destination A.
...
attractivenssDataOutput %>%
  ...
  labs(title="Summary of Attractiveness")+
  mapTheme(5,5)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
```{r calculate distance,  message=FALSE, warning=FALSE, results='hide'}
# # parks
...
```{r centrality, fig.height = 3,message = FALSE, warning = FALSE, results='hide', out.width ='100%',fig.align = 'center',fig.cap = 'Figure5.5.1 Total Vistors of Related Brand With Naics'}
...
```{r calculate Large attractiveness PCA, fig.height = 3,message = FALSE, warning = FALSE, echo=FALSE, out.width ='80%',fig.align = 'center',fig.cap = 'Figure6.2.3 Primary Components of Large Attractiveness'}
...
```{r calculate Small attractiveness PCA, fig.height = 3,message = FALSE, warning = FALSE, echo=FALSE, out.width ='80%',fig.align = 'center',fig.cap = 'Figure6.2.3 Primary Components of Small Attractiveness'}
...
```{r noPCATogo, message=FALSE, warning=FALSE}
...
```{r combined three parameters, message=FALSE, warning=FALSE, results='hide'}
...
```{r annualModelPlot, fig.height = 6,message = FALSE, warning = FALSE,out.width ='100%',fig.align = 'center',fig.cap = 'Figure7.1.1 Histograms of R square in different scenarios'}
...
```{r temporal chart, fig.height = 2,message = FALSE, warning = FALSE,out.width ='100%',fig.align = 'center',fig.cap = 'Figure5.7.1 Temporal Chart of vistor and temperature'}
# temperature data
weather.Data <-
...
```{r Winter R square map, fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure6.7.6 Fitting R square Distribution Map in Winter Model'}
...
print(paste0("MAPE for Scenario 3 is ",round(mean(error$MAPE,na.rm=T),4)))
```
...
```{r add visits frequency level into the model, message=FALSE, warning=FALSE,fig.height = 4,fig.cap = 'Figure 7.3.1 R-squared Distribution Histgram for Optimized Model'}
...
```{r bar plot for errr in different frequency level, fig.cap='Figure 7.3.2 Error among Different Route Visit Frequency Level', fig.height=4, message=FALSE, warning=FALSE}
...
```{r hist of MAPE for cbgS,fig.height = 8,fig.cap = 'Figure7.3.2.1 MAPE Distribution Histgram for Census Block Groups'}
...
```{r MAPE map for cbgs,fig.height = 4,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure 7.3.2.2 MAPE Distribution Map for Census Block Groups'}
  ggplot() + 
...
```{r hist of MAPE for parks,fig.height = 4,fig.cap = 'Figure 7.3.2.3 MAE & MAPE Distribution Histgram for Different Parks & Rec Centers'}
...
```{r MAPE map for parks,fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure7.2.3.4 MAPE Distribution Map for Parks'}
  ggplot() + 
...
```{r CV before optimalization over district, message=FALSE, warning=FALSE, results='hide'}
...
```{r CV after optimalization over district, message=FALSE, warning=FALSE, results='hide'}
...
```{r MAPE map for cv,fig.height = 8, out.width ='100%',fig.cap = 'Figure 7.4.1 MAPE from Cross-Validation for PPR Districts (Left: Before-Optimization, Right:After-Optimization'}
...
plot_grid(m1, m2, 
          # labels = c("MAPE from before-Optimized Model Cross-Validation for PPR Districts",
          #            "MAPE from Optimized Model Cross-Validation for PPR Districts")
          )
```
