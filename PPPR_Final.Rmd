...
kable(cluster2_mean %>% 
        ...
        mutate(group=c("Overprogrammed","Normal", "Potential"),.before = 2),align = 'c',caption = '<center>Table 4.1 Mean values of clusters for conflicts <center/>') %>%
  kable_classic(full_width = T)%>%
  kable_styling(position = "center")
```
...
### Export top 5 potential facilities

sgIntegrated_byMonth_cluster1 <- sgIntegrated_byMonth %>%
  #st_drop_geometry() %>% 
  filter(cluster == 1) %>% 
  group_by(placekey) %>% 
  summarize(month = n(),
...
ggplot() + 
  ...
  labs(title = "", 
       subtitle = "",
       x="",y="")+
  scale_color_manual(values = my_cols,name = "Facility Clusters", label=c("Overprogramed (few visits many permits)","Normal","Potetial (many visits few permits)")) +
  mapTheme()+
  theme(legend.position = "bottom",
        legend.text = element_text( colour = "black", size = 8))
```
