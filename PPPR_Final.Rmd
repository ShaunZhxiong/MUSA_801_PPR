...
The eventually user interface will be the [Dashboard](https://ppr-dashboard-xfw9y.ondigitalocean.app/). That will convey the PPR related information, useful exploratory analysis, and the outcome of market area from the huff model in the end. This dashboard will provide data visualization of their existing programs, permits and estimated number of visits in each facility, service area, and district, as well as display proposed activities to visitors in the future.
...
```{r openData_pprDistrictAndServiceArea, fig.align='center', fig.cap='Figure1.1.1 Location of PPR Districts and Pilot Service Areas', fig.height=8, message=FALSE, warning=FALSE, out.width='80%', results='hide'}
...
In 2021, the recorded program schedules cover seven [categories](https://musa6112022.slack.com/files/U02TPDVLJJZ/F033K006673/pppr_programcategory.pdf) of After School, Athletic, Camp, Cultural, Educational, Environmental/Outdoor, and other activities. There were other activities applied by volunteer or local residents recorded in the permit system. In the following analyses, events refer to the combination of program and permit data.
...
Sometimes, SafeGraph will count "Residents" or "passer-bys" as "visitors", which will result in huge bias and mislead the analysis. So, to conclude **typical position types of SafeGraph data bias**, we choose ***midnight visits***and ***dwelling time*** as factors for **cluster analysis**.
...
In Figure 3.2.9, we calculate the average dwelling time for each facility. From the map, we can find out that the **longest dwelling** time happes at the placekey (zzw-222@628-pg9-tg), the **Clemente Park Playground**.
...
```{r spatially distribution of sg bias clusters,fig.height = 16,message = FALSE, warning = FALSE,out.width ='100%',fig.align = 'center',fig.cap = 'Figure4.1.2 Map of Bias Type of SafeGraph Data'}
...
```{r Smith Center table, message = FALSE, warning = FALSE}
...
```{r Thomas B. Smith Recreation Center dwell time ,fig.height = 4,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure3.3.3 Smith Rec Center Visits in Program Level (data from PPR)'}
...
```{r Thomas B. Smith Recreation Center dwell time2 ,fig.height = 3,message = FALSE, warning = FALSE,out.width ='100%', fig.align = 'center',fig.cap = 'Figure3.3.4 Dwelling time and time distribution of Smith Rec Center Activity (data from SafeGraph)'}
sumVisit <- dwellTime %>% 
...
visitCount %>% 
  ...
  geom_vline(xintercept = (smithProgramDate%>% filter(month %in% c(7,10,11)))$AttendenceRealDate , colour=palette1_assist, size=0.5)+
...
We will use an advanced version of Huff model to predict the probability of visitors at each census block group going to a given park facility. Our Huff model includes neighboring effect, based on the assumptions that: (1) People tend to choose more attractive travel destinations; (2) People tend to choose closer travel destinations; (3) People tend to choose travel destinations with more beneficial future choices. For example, a tourist at Origin O has two destination choices A and B, with equal distance to origin O. In this case, destination B should be preferred since it has more future choices in its neighborhood compared with destination A.

The below is the equation for our Huff Model.

$$
P_{ijt} = \frac{A_{jt}^\alpha*D_{ij}^\beta*C_{jt}^\theta}{\sum_{j=1}^{n}A_{jt}^\alpha*D_{ij}^\beta*C_{jt}^\theta}
$$
$P_{ijt}$ represents the probability of a tourist at location *i* visiting attraction *j* at time *t*; *A*jt is the attractiveness of attraction *j* at time *t* (we will include several attractiveness in our model); $D_{ij}$ is the distance between origin *i* and attraction *j*; $C_{jt}$ is the term used to describe the neighboring effect of attraction *j* (we mainly consider convenient stores), relative to other attractions at time *t*; and *n* indicates the total number of attractions in the area. The parameters $\alpha$, $\beta$, and $\theta$ are three parameters associated with the attractiveness, distance, and neighboring effect factors,for each origin respectively.
...
```{r visualize centrality, fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.5.2 Overlay Map of Parks(black), Convinient Stores(orange)'}
...
ggplot() +
  geom_sf(data=pprServiceArea,color='black',size=0.35,fill= "transparent")+
  geom_sf(data=pprDistrict,color='black',size=0.5,fill= "transparent")+
  geom_sf(data=parksSurroundDot,aes(geometry=geometry),color = "black",fill = "grey",alpha = 1, size = 1) +
  geom_sf(data=convenientSurroundDot,aes(geometry=geometry),color = palette2[1],fill = palette2[1],alpha = 1,size = 2) +
  mapTheme()
```
...
```{r Wrangle sg and property data for model, fig.height = 8,message = FALSE, warning = FALSE, results='hide'}
...
1.  Replace NA value of probability with 4.4e-5 and use PCA from the large group of attractiveness as predictors; <br>
...
2.  **Small group of attractiveness**: some important attractiveness, like area, trail miles, swimming PoolNum, spraygroundNum, tennisCourtNum, treeHeight, treeArea, programNum, and permitNum are included in the PCA. Then 2 prominent components are extracted.
...
```{r model1 fit parameter,message = FALSE, warning = FALSE}
...
outcomeAnalysis(parameterDrop,"SCENARIO 1: Drop NA probability and use PCA from the large group of attractiveness")

```

**SCENARIO 2** Drop NA probability and use PCA from the small group of attractiveness.

```{r, 2model fit parameter, message = FALSE, warning = FALSE}
...
outcomeAnalysis(parameterDrop,"SCENARIO 2: Drop NA probability and use PCA from the small group of attractiveness")
```

**SCENARIO 3** Drop NA probability and use no PCA

```{r huff model3 fit parameter, message = FALSE, warning = FALSE}
# SCENARIO 3: Drop NA probability and use no PCA
...
outcomeAnalysis(parameterDrop,"SCENARIO 3: Drop NA probability and use no PCA")
```
...
outcomeAnalysis(parameterDrop,"SCENARIO 4: Replace NA probability and use PCA from the large group of variables")
```

In the above, we see the R square from different models. The best one is the scenario 3, where we drop NA probability and don't use PCA. But one thing needs to be noted is that scenario 1 also has a similar R square with scenario 3. 
...
```{r programNum as independent predictor}
...
need modification
```{r sCENARIO 1 - CV over district}
source("huffModelScripts.R")

CVModelData <- RealModelData %>% 
...
cv_result  <- huff_crossValidate(data = CVModelData,
                                         cv_id = "PPR_DISTRICT",
                                         places = RealModelPlaces,
                                         neighbor_data = convenientSurroundEffect,
                                         neighbor_id_column="placekey",
...
cv_result <- left_join(cv_result,
                       CVModelData %>% dplyr::select(origin, OBJECTID, probability, PPR_DISTRICT),
...
need modification
```{r add visits frequency level into the model}
CVModelData2 <- RealModelData %>% 
...
# drop_list <- CVModelData2 %>% 
#   group_by(origin) %>% 
#   summarise(n_program = n_distinct(programNum),
#             n_frequency = n_distinct(frequency)) %>% 
#   filter(n_program < 2 | n_frequency < 2)
# 
# CVModelData2 <- CVModelData2 %>% 
#   filter(!origin %in% drop_list$origin)

# Split data
set.seed(3456)
split <- partition(CVModelData2$origin, p=c(train=0.5, test=0.5))
Train <- CVModelData2[split$train,]
Test <- CVModelData2[split$test,]

set.seed(3456)
inTrain <- createDataPartition(
              y = paste(CVModelData2$origin, CVModelData2$frequency, CVModelData2$programNum), 
              p = .50, list = FALSE)
Train <- CVModelData2[inTrain,] 
Test <- CVModelData2[-inTrain,]  
...
# join to same index with Test Set
parameter_full <- left_join(Test %>% dplyr::select(origin), 
                            parameter, 
                            by="origin") 

source("huffModelScripts.R")
predictionResult <- huff_predict(data = Test,
                           places = testPlaces,
                           neighbor_data = convenientSurroundEffect,
                           neighbor_id_column="placekey",
                           neighbor_attr_column ="visits",
...
predictionResult_3 <- predictionResult_2 %>% 
  left_join(.,CVModelData2 %>% dplyr::select(origin, OBJECTID, frequency),
            by=c("origin"="origin","id"="OBJECTID"))
...
predictionResult_group <- predictionResult_group %>% 
  filter(MAPE<100)

cv_result  <- huff_crossValidate(data = CVModelData2,
                                         cv_id = "frequency",
                                         places = RealModelPlaces,
                                         neighbor_data = convenientSurroundEffect,
                                         neighbor_id_column="placekey",
...
cv_result <- left_join(cv_result,
                       CVModelData2 %>% dplyr::select(origin, OBJECTID, probability, frequency),
                       by = c("origins_name"="origin", "destinations_name"="OBJECTID")) %>% 
  mutate(error = probability - huff_probability)
