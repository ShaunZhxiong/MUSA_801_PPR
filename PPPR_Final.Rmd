...
ggplot(data = depaCountPlotHF) + 
  ...
  geom_sf(data=pprDistrict %>% st_transform(crs=4326),fill ="transparent", color="black",size=1)+
  geom_sf(data=propertyWithPlaceKey %>% dplyr::select(OBJECTID) %>%distinct() %>%  st_transform(crs=crs), fill ='#C5C5C5', color='#C5C5C5',alpha=0.3,size=0)+
  geom_sf(data=CensusPoly %>% filter(GEOID %in% depaCountPlotHF$departure),fill ="transparent", color=palette1_main,size=1)+
  geom_curve(aes(x = departure.x, y = departure.y, xend = park.x, yend = park.y,color = q5(visitors)),
             # arrow = arrow(length = unit(0.01, "npc"), type="closed"),
             size = 1,curvature = -0.2, alpha = 0.7)+
  scale_color_manual(values = palette5,
                     labels = qBr(depaCountPlotHF,"visitors"),
                     name = "Device Visits (Quintile Breaks)") +
  labs(x="",y="")+
  mapTheme()+
  theme(legend.position = "bottom",panel.spacing = unit(6, 'lines'),
...
#### 4.1 Conflicts between Mobility Data and Position Usage
...
# integrate dwell, total counts and permit, programs
sgIntegrated_byMonth <- full_join(dwellMean %>% dplyr::select(placekey,month,meanDwell), 
  ...
  na.omit() %>% 
  ungroup() %>% 
  left_join(propertyWithPlaceKey %>% dplyr::select(placekey, OBJECTID, PPR_DISTRICT)) %>% 
  ## join permits
  ...
  ## join programs
  left_join(program2021.join %>% 
              group_by(PPR_Properties_ObjectID) %>% 
              summarize(programs = n()),
            by=c("OBJECTID"="PPR_Properties_ObjectID")) %>% 
  na.omit() %>% 
  dplyr::select(-OBJECTID) %>% 
  group_by(placekey, month, PPR_DISTRICT) %>% 
  summarize(programs= sum(programs),
           permits = sum(permits),
           totalEvents = sum(permits,programs),
            visits = mean(visits),
...
sgIntegrated_byMonth.scale <- scale(sgIntegrated_byMonth %>% 
                                      dplyr::select(-placekey,-month,-PPR_DISTRICT, -permits, -totalEvents) %>% 
                                      st_drop_geometry())
...
# Run K-Means cluster
set.seed(1234)
cluster2 <- kmeans(sgIntegrated_byMonth.scale, centers = 3, nstart = 5) 
# summary(cluster2)
...
cluster2_mean <- aggregate(sgIntegrated_byMonth %>% 
                             dplyr::select(-placekey, -cluster,-month,-PPR_DISTRICT) %>% 
                             st_drop_geometry(),
...
kable(cluster2_mean %>% 
        mutate(color=c("blue","orange","pink"),.before = 2) %>% 
        mutate(group=c("Potential", "Normal", "Overprogrammed"),.before = 2),align = 'c',caption = '<center>Table 4.1 Mean values of clusters for conflicts <center/>') %>%
  kable_classic(full_width = T)%>%
  kable_styling(position = "center")
```
...
#Color points by groups
my_cols <- c(palette5[5],palette5[1],palette1_main)

pairs(sgIntegrated_byMonth %>% 
        st_drop_geometry() %>% 
        dplyr::select(meanDwell,visits,programs), 
      pch = 19, cex = 0.5, cex.labels=1, diag.panel = panel.hist,
      size=0.5,
...
The Figure 4.1.1 shows how these three types of facilities are spatially distributed in District 7,8,9. There are many over-programmed facilities in the south Philadelphia.

```{r find the case, include=FALSE, eval=FALSE}
# Checked the cluster number but find 4 facilities (22n-222@628-pgj-2x5, zzw-222@628-php-s89, zzw-222@628-pmb-cdv, zzy-222@628-pgm-4sq) from potential should belong to over-programmed facilities. So I change the as below manually.

sgIntegrated_byMonth2 <- copy(sgIntegrated_byMonth)
 
                                                          sgIntegrated_byMonth2$cluster[which(
                                                            sgIntegrated_byMonth$placekey == "22n-222@628-pgj-2x5"|                          sgIntegrated_byMonth$placekey == "zzw-222@628-php-s89"|                          sgIntegrated_byMonth$placekey == "zzw-222@628-pmb-cdv"|
sgIntegrated_byMonth$placekey == "zzy-222@628-pgm-4sq")] <- 3

# Potential Facilities
sgIntegrated_byMonth2 %>%
  st_drop_geometry() %>% 
  filter(cluster == 1) %>% 
  group_by(placekey) %>% 
  summarize(howManyMonthBelongToThisCluster=n()) %>% 
  kable(.,align = 'c',caption = '<center>Table 4.1.2 Mean values of clusters for conflicts <center/>') %>%
  kable_classic(full_width = F)%>%
  kable_styling(position = "center")%>%
  scroll_box(width = "100%", height = "400px")

# Over-programmed facilities
sgIntegrated_byMonth2 %>%
  st_drop_geometry() %>% 
  filter(cluster == 3) %>% 
  group_by(placekey) %>% 
...
# More details in cluster1 Potential
sgIntegrated_byMonth_cluster1 <- sgIntegrated_byMonth2 %>%
  st_drop_geometry() %>% 
  filter(cluster == 1) %>% 
  group_by(placekey) %>% 
  summarize(month = n(),
            ...
            visits = sum(visits))
# More details in cluster2 Normal
sgIntegrated_byMonth_cluster2 <- sgIntegrated_byMonth2 %>%
  st_drop_geometry() %>% 
  filter(cluster == 2) %>% 
  group_by(placekey) %>% 
  summarize(month = n(),
            programs = sum(programs),
            visits = sum(visits))
# More details in cluster3 Over-programmed
sgIntegrated_byMonth_cluster3 <- sgIntegrated_byMonth2 %>%
  st_drop_geometry() %>% 
  filter(cluster == 3) %>% 
  group_by(placekey) %>% 
  summarize(month = n(),
...
ggplot() + 
  geom_sf(data=sgIntegrated_byMonth2 %>%
            group_by(placekey,cluster) %>% 
            summarize(howManyMonthBelongToThisCluster=n()) %>% 
            filter(howManyMonthBelongToThisCluster >=1),aes(color=as.factor(cluster)), size = 1)+
  geom_sf(data=pprDistrict,color="black",size=0.7,linetype ="dashed",fill = "transparent")+
  geom_sf(data=pprDistrict %>% filter(DISTRICTID %in% c(7,8,9)),color="black",size=1,fill = "transparent")+
  labs(title = "", 
       subtitle = "",
       x="",y="")+
  scale_color_manual(values = my_cols,name = "Facility Clusters", label=c("Potetial (many visits few permits)","Overprogramed (few visits many permits)","Normal")) +
  mapTheme()+
  theme(legend.position = "bottom",
        legend.text = element_text( colour = "black", size = 8))
```

#### 4.2 Special Cases1: Thomas B. Smith Recreation Center
...
#   na.omit()%>%
#   ggplot(aes(visitDay,visits)) +
#   geom_line(color=palette1_assist,size=0.75)+
#   geom_vline(xintercept = albertChristy$AttendenceRealDate, colour=palette1_main, linetype = "longdash")+
#   labs(title = '',x="Visit Date",y="Safegraph Visit")+
#   plotTheme()+
#   theme(panel.border = element_rect(colour = "black", fill=NA, size=1))
...
#### 4.3 Special Cases2:Albert W. Christy, Sr. Recreation Center

**Albert W. Christy, Sr. Recreation Center** is a representative case of **potential** facilities with low permits, high visits and long dwelling time.It is a neighborhood park in District 8. There are only two types of programs scheduled, football and after school. But 1000+ device visits was recorded by the SafeGraph data on a single day, like some dates in the middle of March, June, July and August.[Click for more Information](https://www.phila.gov/2021-09-28-new-amenities-at-christy-rec-center-attract-teens/)
...
According to news about the Christy Rec Center, the weekly program attracts an average of 50-100 teens a week, and is contributing to the overall increase in recreational programming now available to older youth in the West Philadelphia neighborhood. Limited types of programs were provided based on PPR record.

```{r Christy Recreation Center dwell time ,fig.height = 4,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure3.3.6 Christy Rec Center Activity with Total'}
...
```{r GEP temperal pattern by day ,fig.height = 3,message = FALSE, warning = FALSE,out.width ='100%',fig.align = 'center',fig.cap = 'Figure3.3.7 Distribution of  Christy Recreation Center visits in 2021'}
...
A baseball court, playground, and an outdoor basketball court surround the Christy Rec Center. Interior basketball court, multi-purpose interior space are provided inside the Christy Rec Center.  
...
```{r GEP temperal pattern by hour,fig.height = 3,message = FALSE, warning = FALSE,out.width ='100%',fig.align = 'center',fig.cap = 'Figure3.3.8 Temperal Vistis Pattern By Hour & Vistis Dwelling Time'}
sumVisit <- visitHour %>% 
  filter(placekey=='zzz-222@63s-dvq-5fz') %>% 
  dplyr::select(-month) %>% 
  group_by(hour) %>%
...
According to the Figure3.3.8 above, people usually dwell longer (\>120min) here, because most activities, like athletic events and after class activities take place during the day are long. It seems 11 am to 3 pm is the most time period with the most aggregated visits.

The facility improvements in 2021 make the Christy Recreation Center a safe destination for West Philadelphia teens and young adults. It is a good opportunity to see the community and local businesses respond to violence by coming together with law enforcement and elected leaders to expand on- and off-court programs for teens and older youth. More programs related basketball and more interior activities might be suggested for future program schedules.


### 5 Huff Model with Neighbor Effect

The original Huff Model (Huff, 1964) is designed to estimate the probability of customers at each origin patronizing a given store among all stores as their destination choices. Two factors are taken into account: 1. Attractiveness 2. Distance. Attractiveness can be computed as a function of many attributes of a store, including the store size, number of parking spaces, customer reviews,
etc.

We will use an advanced version of Huff model to predict  the probability of visitors at each census block group going to a given park facility. A socially aware Huff model include social factor and neighboring effect, based on the
assumptions that: (1) People tend to choose more attractive travel destinations; (2) People tend to choose closer travel destinations; (3) People tend to choose travel destinations with more beneficial future choices. For example, a tourist at Origin O has two destination choices A and B, with equal distance to origin O. In this case, destination B should be preferred since it has more future choices in its neighborhood compared with destination A.
...
#st_write(attracGEO, "data/output/attracGEO.GeoJSON")
...
attracGEO <- 
  st_join(attractivenssDataOutput, 
          pprDistrict %>% dplyr::select(DISTRICTID),
          left=TRUE) %>% 
...
attracGEOVISIT <- modelData %>% 
  ...
  group_by(placekey) %>% 
  summarise(visitors=sum(visitors)) %>% 
  inner_join(attracGEO %>% st_drop_geometry(),by="placekey")
...
corrplot(attracGEO.cor,method="number", tl.cex = 2, type = "lower",number.cex=2,addgrid.col = "black",addCoef.col="black")
```
...
pcaDF <- attracGEOVISIT %>% dplyr::select(-visitors,-placekey, -DISTRICTID)
...
# try no pca
noPCATogo <- attracGEO %>% dplyr::select(-DISTRICTID) %>% st_sf()
...
ggplot() +
  geom_sf(data=pprServiceArea,color='black',size=0.35,fill= "transparent")+
  geom_sf(data=pprDistrict,color='black',size=0.5,fill= "transparent")+
  geom_sf(data=parksSurroundDot,aes(geometry=geometry),color = "black",fill = "grey90",alpha = 1, size = 1) +
...
huffData <- 
  ...
  left_join(prob,by=c("placekey","origin"))
...
huffDataDropNaNOPCA <- 
  huffDataDropNa%>% 
  left_join(noPCATogo %>% st_drop_geometry(),by="placekey")
```
...
Train <- RealModelData3 %>% 
  filter(placekey %in% TrainList)
# flter test data
Test <- RealModelData3 %>% 
  filter(placekey %in% TestList)

# fit on train test
fit3 <- fit_parameter(data = Train,
                           # remove some places
                           places = placeSet %>% filter(setID=="Train"),
                           neighbor_data = convenientSurroundEffect,
                           neighbor_id_column="placekey",
                           neighbor_attr_column ="visits",
                           id_column="placekey",
...
huff_results <- huff_NE(destinations_name = Test$id, 
                        destinations_attractiveness = Test[,5:21], 
