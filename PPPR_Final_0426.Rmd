...
By incorporating this novel dataset, we can help the PPR analyze if their programming meet citizens' demands and to better assign their program resources with SafeGraph's Pattern dataset. With understanding of the imbalance between demand and supply, we can adjust the quantity of programs and events, like increasing number in low-supplied facilities and reducing number in over-supplied facilities. The prediction outcome of market area can be used to suggest PPR's future programming strategies

In order to know how many additional programs are needed and how the adjustment will affect other facilities, we will refer to the Huff Model. In spatial analysis, the Huff model is a widely used tool for predicting the probability of consumers visiting a site, as a function of the distance of the site, its attractiveness, and the relative attractiveness of alternatives. With the predicted probability of consumers visiting a facility, we could interpret and normalize it to reflect the quantity of visitors from a census block group to a certain facility. In that case, we can help the PPR better understand the use of their facilities and provide recommendations on how to allocate programming resources better within Program Service Areas and the types of programs they should offer to meet diverse user demands.
...
With SafeGraph's Pattern dataset, we can find visit counts from device panel are inconsistent with their scheduled programs and events in some facilities.
...
In terms of helping allocate programs and resources, we **divided all locations into 3 categories** based on their **visit data** and **activity scheduled with programming**. (The number of facilities with both of program scheduled and safegraph data available is 36). Refer to Table 3.3 and Figure3.3.1.

***Potential facilities***: They are shown as two **pink** points where **huge amount of visitors** who will **stay longer** than normal, but only **relatively few activity programs** here, which means there is great **potential** to develop activities, and PPR may set more programs in the future. 

***Normal facilities***: 25 **Orange** points are **normal** positions with foot traffic that is **consistent** with the quantity of programs.

***Over-programmed facilities***: They are shown as nine **blue** points where **many programs** but **few foot traffic**, since they are usually **community centers** who support **group events**.
...
ggplot() + 
  geom_sf(data=sgIntegrated_byMonth %>% 
          ...
          color = palette1_assist, size=5,alpha = 0.4)+
  geom_sf(data=sgIntegrated_byMonth %>% 
            filter(cluster %in% c(1,2)) %>%
            left_join(propertyWithPlaceKey,by=c("OBJECTID")) %>% st_sf() %>% st_centroid(),
          aes(color=as.factor(cluster)), size=5)+
...
#### 4.4 Special Cases2:Albert W. Christy, Sr. Recreation Center

**Albert W. Christy, Sr. Recreation Center** is a representative case of **potential** facilities with low permits, high visits and long dwelling time.It is a neighborhood park in District 8. There are only 33 programs scheduled in the last year, but 1000+ device visits was recorded by the SafeGraph data on a single day.[Click for more Information](https://www.phila.gov/2021-09-28-new-amenities-at-christy-rec-center-attract-teens/)

```{r Christy img, echo=FALSE,fig.height = 2,out.width = '80%',fig.align = 'center'}
knitr::include_graphics(c("resources/imgaes/ChristyRec_combine1.jpg"))
```
...
```{r Christy Recreation Center dwell time ,fig.height = 3,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure3.3.6 Christy Rec Center Activity with Total'}
...
```{r Christy temperal pattern by day2 ,fig.height = 3,message = FALSE, warning = FALSE,out.width ='100%',fig.align = 'center',fig.cap = 'Figure3.3.7 Distribution of  Christy Recreation Center visits in 2021'}
...
```{r Christy img2, echo=FALSE,fig.height = 2,out.width = '80%',fig.align = 'center'}
knitr::include_graphics(c("resources/imgaes/ChristyRec_combine2.jpg"))
```
...
```{r Christy temperal pattern by hour,fig.height = 3,message = FALSE, warning = FALSE,out.width ='100%',fig.align = 'center',fig.cap = 'Figure3.3.8 Temperal Vistis Pattern By Hour & Vistis Dwelling Time'}
...
```{r calculate attractiveness, message=FALSE, warning=FALSE, echo=FALSE, results='hide', fig.height = 12,out.width ='200%',fig.align = 'center',fig.cap = 'Figure5.1.1 Summary of Attractiveness' }
attractivenssData <- 
  modelData %>% 
...
```{r calculate correlation, fig.height = 8,message = FALSE, warning = FALSE, echo=FALSE, out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.1.2 Correlation of Attractiveness'}
...
attracGEOVISIT_norm <- as.data.frame(lapply(attracGEOVISIT %>% 
                      dplyr::select(-OBJECTID, -PPR_DISTRICT) %>% 
                      mutate_all(.,as.numeric), min_max_norm))

attracGEO.cor = cor(attracGEOVISIT_norm, method="spearman")
...
```{r calculate Large attractiveness PCA, fig.height = 8,message = FALSE, warning = FALSE, echo=FALSE, out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.1.3 Primary Components of Large Attractiveness'}
...
corrplot(t(pca$rotation[,1:3]), is.corr=FALSE)


# -----------Large Attractiveness Except program programNum programCap----------------
pcaDF2 <- attracGEOVISIT %>% 
  dplyr::select(-visitors,-OBJECTID, -PPR_DISTRICT, -programNum, -programCap) %>% 
  mutate_all(.,as.numeric)
pca2 = prcomp(pcaDF2, center = TRUE, scale. = TRUE)  

#decide the number of components
# components = 1:ncol(pcaDF2)
# plot(pca2$sdev ~ components, ylab = "PCA Standard Deviation", xlab = "Component", pch = 19, type = "b")
# abline(v = 9, col = "red")
# fa.parallel(scale(pcaDF2), fa = 'pc', n.iter = 100, show.legend = FALSE)

# pca2$rotation <- -1*pca2$rotation
# pca2$x <- -1*pca2$x
# 
# corrplot(t(pca2$rotation[,1:3]), is.corr=FALSE)
...
```{r calculate Small attractiveness PCA, fig.height = 8,message = FALSE, warning = FALSE, echo=FALSE, out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.1.4 Primary Components of Small Attractiveness'}
# -----------Small Attractiveness--------------
pcaDFSmall <- 
  attracGEOVISIT %>% 
  dplyr::select(area,trailMiles, facilityNum,facilityArea,hydrationNum,tennisCourtNum,playgroundNum,programNum,permitNum) %>% 
  mutate_all(.,as.numeric)
...
LargePCAAttTogo2 <- 
  cbind(LargePCA2, attracGEO %>% dplyr::select(OBJECTID)) %>% 
  st_sf() %>% 
  mutate(PC1 = scales::rescale(PC1, to=c(0,1)),
         PC2 = scales::rescale(PC2, to=c(0,1)),
         PC3 = scales::rescale(PC3, to=c(0,1)))
...
```{r Wrangle sg and property data for model, fig.height = 8,message = FALSE, warning = FALSE, results='hide',out.width = '80%',fig.align = 'center',fig.cap = 'Figure5.3.1 Census Block Group with Missing Data'}
...
```{r centrality, fig.height = 3,message = FALSE, warning = FALSE, echo=FALSE, results='hide', out.width ='100%',fig.align = 'center',fig.cap = 'Figure5.4.1 Total Vistors of Related Brand With Naics'}
...
```{r visualize centrality, fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.4.2 Overlay Map of Parks(green), Restaurants(red) and Convinient Stores(orange)'}
...
ggplot() +
  geom_sf(data=parksSurroundDot,aes(geometry=geometry),color = "#e0f3db",fill = "#e0f3db",alpha = 1, size = 1)+
  geom_sf(data=pprServiceArea,color='black',size=0.35,fill= "transparent")+
  geom_sf(data=pprDistrict,color='black',size=0.5,fill= "transparent")+
...
We prepared 4 sets of data to test the Huff model:
...
```{r model1 fit parameter, fig.height = 3,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.5.1 Histogram of data set1'}
...
```{r export fitted data for application}

# sCENARIO 1 variant: Drop NA probability and use large PCA with programNum outside PCA
RealModelData <- huffDataDropNaLargeAttract_program
RealModelPlaces <- attracGEO %>% dplyr::select(OBJECTID) %>% st_centroid()
...
dataForPrediction <- left_join(dataForPrediction, parameter, by="origin") 

#st_write(dataForPrediction, "data/output/dashboard/final/dataForPrediction.csv")


R_Square <- parameterDrop$r2 
hist(R_Square,breaks=40)

outcomeAnalysis(parameterDrop,"sCENARIO 1 variant: Drop NA probability and use large PCA with programNum outside PCA")
...
# replace with actual probability
predictionResult <- predictionResult %>% 
  dplyr::select(-huff_probability) %>% 
  left_join(dataForPrediction %>% dplyr::select(origin, id, prob),
            by=c("origins_name"="origin", "destinations_name"="id")) %>% 
  rename("huff_probability"="prob")
#st_write(predictionResult, "data/output/dashboard/final/predictionResult.csv")
```


**SCENARIO 2** Result (Drop NA probability and use small PCA)

```{r, 2model fit parameter, fig.height = 3,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.5.2 Histogram of data set2'}
...
```{r huff model3 fit parameter, fig.height = 3,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.5.3 Histogram of data set3'}
...
```{r model huff4 fit parameter, fig.height = 3,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.5.4 Histogram of data set4'}
...
```{r temporal chart1, fig.height = 4,message = FALSE, warning = FALSE,out.width ='100%',fig.align = 'center',fig.cap = 'Figure5.6.1 Temporal Chart of vistor and temperature'}
# temperature data
weather.Data <-
...
```{r missing data for model, fig.height = 6,message = FALSE, warning = FALSE, results='hide',out.width = '80%',fig.align = 'center',fig.cap = 'Figure5.6.2 Census Block Group with Missing Probability'}
# calculate probability 
...
```{r Spring R square map, fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.6.3 Fit R square in Spring Model Distribution Map'}
...
```{r Summer R square map, fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.6.4 Fit R square in Summer Model Distribution Map'}
...
```{r Autumn R square map, fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.6.5 Fit R square in Autumn Model Distribution Map'}
...
```{r Winter R square map, fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.6.6 Fit R square in Winter Model Distribution Map'}
...
```{r annual R square map, fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.6.7 Fit R square in Annual Model Distribution Map'}
...
#### 5.7 Train and Test in SCENARIO 1

```{r predict in sCENARIO 1}
...
error_major <- left_join(huff_results, Test, by=c("origins_name"="origin", "destinations_name"="id") ) %>% 
  mutate(AE = abs(huff_probability - probability),
         APE = abs(huff_probability - probability)/probability) %>% 
  filter(probability>0.1) %>% 
  group_by(origins_name) %>% 
...
mean(error$MAPE,na.rm=T)
mean(error$MAE,na.rm=T)

mean(error_major$MAPE,na.rm=T)
mean(error_major$MAE,na.rm=T)

```

```{r hist of MAPE for cbgS in sCENARIO 1,fig.height = 8}
hist(error %>% filter(MAPE<=10) %>% pull(MAPE), breaks=100, 
     xlab="MAPE", main="Distribution Histogram of MAPE for Different Block Groups")
```


```{r MAPE map for cbgs in sCENARIO 1,fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure MAPE Distribution Map for Census Block Groups'}

  ggplot() + 
    geom_sf(data=left_join(error, CensusPoly, by=c("origins_name"="GEOID")) %>% st_sf(),
            color="black",size=0.05,aes(fill = q5(MAPE)))+
    geom_sf(data=pprDistrict %>% st_union(),color="black",size=0.5,fill = "transparent")+
    scale_fill_manual(values = palette5, name="MAPE", label=qBr(error,"MAPE",F))+
    mapTheme()+
    theme(legend.position = "bottom")
```
```{r hist of MAPE for parks in sCENARIO 1,fig.height = 8}
error_parks <- left_join(huff_results, Test, by=c("origins_name"="origin", "destinations_name"="id") ) %>% 
  mutate(AE = abs(huff_probability - probability),
         APE = abs(huff_probability - probability)/probability) %>% 
  group_by(destinations_name) %>% 
  summarise(MAPE = mean(APE,na.rm=T),
            MAE = mean(AE,na.rm=T))

mean(error_parks$MAPE,na.rm=T)
mean(error_parks$MAE,na.rm=T)

error_parks_major <- left_join(huff_results, Test, by=c("origins_name"="origin", "destinations_name"="id") ) %>% 
  mutate(AE = abs(huff_probability - probability),
         APE = abs(huff_probability - probability)/probability) %>% 
  filter(probability>0.1) %>% 
  group_by(destinations_name) %>% 
...
mean(error_parks_major$MAPE,na.rm=T)
mean(error_parks_major$MAE,na.rm=T)


hist(error_parks %>% filter(MAPE<=10) %>% pull(MAPE), breaks=100, xlab="MAPE", main="Distribution Histogram of MAPE for Parks")
```



```{r MAPE map for parks in sCENARIO 1,fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure MAPE Distribution Map for Parks'}

  ggplot() + 
    geom_sf(data=left_join(error_parks, pprProperties, by=c("destinations_name"="OBJECTID")) %>% st_sf(),
            color="black",size=0.05,aes(fill = q5(MAPE)))+
    geom_sf(data=pprDistrict %>% st_union(),color="black",size=0.5,fill = "transparent")+
...
#### 5.8 Train and Test in SCENARIO 3
...
# sCENARIO 3: Drop NA probability and use no PCA
#mutateList <- colnames(huffDataDropNaNOPCA %>% dplyr::select(-origin, -OBJECTID, -probability))
...
In this research, we unveiled the conflicts between visit pattern from Safegraph device panel and scheduled programs from PPR pattern with clustering regression, which indicates imbalance between demand and supply. To provide optimized solutions, we used a novel method to predict the probability of visitors at each census block group going to a given park facility with Huff model. By changing the number of programs, the Huff model can predict future visit flow, which help PPR reallocate resources more reasonable. We divided the data based on record dates into four seasons and run individual model to predict the probability in each season. So PPR staffing can refer them to adjust number of programs, predict the trend of visitors, and arrange resources for the next season.

So far, the limitation of the research is that some data is missing after assigning to four seasons due to limited device pattern from Safegraph, and the average MAPE (Mean Absolute Percentage Error) is relatively high. In general, models are more accurate for frequently visited routes, denser residential block groups, large parks, and parks in central city.
