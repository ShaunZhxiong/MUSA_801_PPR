...
In terms of helping allocate programs and resources, we **divided all locations into 3 categories** based on their **visit data** and **activity scheduled with permits**. (The number of facilities with both of program scheduled and safegraph data available is 36). Refer to Table 3.3 and Figure3.3.1.

***Potential facilities***: They are shown as two **pink** points where **huge amount of visitors** who will **stay longer** than normal, but only **relatively few activity permits** here, which means there is great **potential** to develop activities, and PPR may set more programs in the future. 

***Normal facilities***: 25 **Orange** points are **normal** positions with foot traffic that is **consistent** with the quantity of activities.

***Over-programmed facilities***: They are shown as nine **blue** points where **many permits** but **few foot traffic**, since they are usually **community centers** who support **group events**.
...
```{r visualize centrality, fig.height = 8,message = FALSE, warning = FALSE,out.width ='80%',fig.align = 'center',fig.cap = 'Figure5.4.2 Overlay Map of Parks(black), Restaurants(red) and Convinient Stores(orange)'}
