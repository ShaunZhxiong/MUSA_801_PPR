...
```{r Wrangle property data for model}
...
# get point geometry
modelData <- modelData %>% 
  left_join(parks %>% dplyr::select(placekey)) %>% 
  st_sf() %>% 
  st_transform(crs='EPSG:2272')

modelData <- modelData %>% 
  filter(!st_is_empty(modelData)) 
```

```{r huff model script}
find_neighbor <- function(k, places){
  library(FNN)
  places_matrix <- as.matrix(places %>% 
                               st_coordinates())
  result_matrix <- get.knn(places_matrix, k)$nn.dist
  result <- as.data.frame(result_matrix)
  return(result)
}

data <- find_neighbor(2, modelData)
```
