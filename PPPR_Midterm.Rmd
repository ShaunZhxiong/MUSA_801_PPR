...
For events in PPR facilities, there are two types. The first type we call program is the event that is officially organized by PPR staffs. The second type, permit, is the event that is organized by volunteers and got approvals from PPR. In the following analyses, events refer to the combination of the programs and the permits.
...
The Figure3 is the locations of facilities with events in District 7,8,9. The color red is used to represent the facilities with programs. And the color orange is used to represent the facilities with permits. From the map, we can see that permits cover larger area than programs which indicates the demand exceeds PPR programming scope and the ways to improve PPR service.
