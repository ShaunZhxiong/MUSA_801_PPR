...
modelPlaces <- modelPlaces %>% 
  mutate(attr = sample(1:50, size=nrow(modelPlaces), replace = T))
## park info
modelData <- st_read("data/output/modelData.csv") 
CensusData <-
  get_acs(geography = "block group",
          variables = c("B01003_001E"),
          year=2019, state="PA", county="Philadelphia", geometry=T, output="wide") %>%
  ...
  rename("originGeometry" = "geometry")
modelData <- modelData %>% 
  filter(origin %in% as.list(CensusData$GEOID))%>% 
...
### Model Function
fit <- fit_parameter(data = modelData %>% filter(placekey!="zzw-222@628-pj6-9pv"), 
                           places = modelPlaces, 
